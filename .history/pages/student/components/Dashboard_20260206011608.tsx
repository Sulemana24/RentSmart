...
const Dashboard: React.FC = () => {
  ...
  // Simple toast simulation (you can replace with react-hot-toast for better UI)
    ...
    <div className="relative space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {dashboardStats.map((stat, index) => (
          <Link key={index} href={stat.path} passHref>
            <a
              onClick={() => showToast(`Navigating to ${stat.title}`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pr-6">
              ...
              <Link href={activeTab === "bookings" ? "/bookings" : "/payments"} passHref>
                <a className="text-xs font-bold text-[#FF4FA1] hover:underline">
                  View All
                </a>
              </Link>
            </div>
              ...
              {activeTab === "bookings" ? (
                <div className="space-y-4 animate-in slide-in-from-left-4 duration-300">
                  {bookings.map((booking) => (
                    <Link key={booking.id} href={`/bookings/${booking.id}`} passHref>
                      <a className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl hover:ring-2 ring-[#00CFFF]/20 hover:bg-white dark:hover:bg-gray-700 transition-all cursor-pointer group">
                ...
                <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                  {payments.map((payment) => (
                    <Link key={payment.id} href={`/payments/${payment.id}`} passHref>
                      <a className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border-l-4 border-[#00CFFF] hover:bg-white dark:hover:bg-gray-700 transition-all cursor-pointer">
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <Link key={index} href={action.path} passHref>
                  <a
                    onClick={() => showToast(`Navigating to ${action.label}`)}
          ...
          <Link href="/rooms" passHref>
            <a
              onClick={() => showToast("Viewing occupancy rate")}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <Link href="/maintenance" passHref>
          <a className="w-full md:w-auto px-8 py-3 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:shadow-xl active:scale-95 transition-all">
            Review Requests
          </a>
        </Link>
      </div>
