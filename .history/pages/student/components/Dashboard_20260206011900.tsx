...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="relative space-y-6 animate-in fade-in duration-500 pb-10 px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {dashboardStats.map((stat, index) => (
          <Link 
            key={index} 
            href={stat.path}
            onClick={() => showToast(`Opening ${stat.title}`)}
            className="bg-white dark:bg-gray-800 text-left rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all group block"
          >
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <FiArrowRight className="text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity translate-x-[-10px] group-hover:translate-x-0 transition-all" />
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
            <div className="flex flex-col sm:flex-row items-center justify-between border-b border-gray-100 dark:border-gray-700 pr-0 sm:pr-6">
              <div className="flex w-full sm:w-auto">
                <button
                  onClick={() => setActiveTab("bookings")}
                  className={`flex-1 sm:flex-none px-6 py-4 text-sm font-bold transition-colors ${activeTab === "bookings" ? "text-[#00CFFF] border-b-2 border-[#00CFFF]" : "text-gray-400 hover:text-gray-600"}`}
                >
                  Recent Bookings
                </button>
                <button
                  onClick={() => setActiveTab("payments")}
                  className={`flex-1 sm:flex-none px-6 py-4 text-sm font-bold transition-colors ${activeTab === "payments" ? "text-[#00CFFF] border-b-2 border-[#00CFFF]" : "text-gray-400 hover:text-gray-600"}`}
                >
                  Recent Payments
                </button>
              </div>
              <Link 
                href={activeTab === "bookings" ? "/bookings" : "/payments"}
                className="hidden sm:block text-xs font-bold text-[#FF4FA1] hover:underline p-4"
              >
                View All
              </Link>
            </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <Link 
                  key={index} 
                  href={action.path}
                  onClick={() => showToast(`Opening ${action.label}`)}
                ...
                >
                  ...
                  <div className="text-[10px] font-black text-gray-600 dark:text-gray-300 uppercase tracking-tighter">{action.label}</div>
                </Link>
              ))}
            </div>
          </div>
          ...
          <Link 
            href="/rooms"
            onClick={() => showToast("Viewing detailed occupancy")}
            className="bg-gradient-to-br from-[#00CFFF] to-[#009fbf] rounded-2xl p-6 text-white shadow-lg shadow-[#00CFFF]/20 cursor-pointer hover:brightness-110 transition-all group block"
          >
            <div className="flex items-center justify-between mb-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <Link 
          href="/maintenance"
          className="w-full md:w-auto px-8 py-3 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-xl text-xs font-black uppercase tracking-widest text-center hover:shadow-xl active:scale-95 transition-all"
        >
          Review Requests
        </Link>
      </div>
