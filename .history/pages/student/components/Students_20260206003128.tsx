...
import { 
  ...
  FiX, FiChevronRight, FiFilter, FiUserPlus
} from "react-icons/fi";
...
// Generate 100 sample rooms for the preview
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white dark:bg-gray-800 p-8 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-200/50 dark:shadow-none">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-[#FF4FA1] font-black uppercase text-xs tracking-[0.2em]">
             <FiZap className="fill-current" /> Demo Preview Mode
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-gray-100/50 dark:bg-gray-800/50 p-2 rounded-3xl">
        ...
        <div className="flex gap-2">
          <div className="relative flex items-center">
            <FiFilter className="absolute left-5 text-[#00CFFF]" />
            <select 
              className="pl-12 pr-10 py-5 bg-white dark:bg-gray-800 rounded-[1.5rem] font-black text-xs uppercase tracking-widest border-none shadow-sm outline-none cursor-pointer appearance-none min-w-[180px]"
              value={filterStatus}
          ...
          >
            ...
            <div className="flex flex-col h-full">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-black text-gray-900 dark:text-white truncate max-w-[180px]">{student.name}</h3>
            ...
            <form onSubmit={handlePreviewSubmit} className="p-10 overflow-y-auto space-y-8 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <input required type="text" placeholder="e.g., John Doe" className="w-full px-8 py-5 bg-gray-100 dark:bg-gray-800 rounded-2xl outline-none focus:ring-4 ring-[#FF4FA1]/10 font-bold transition-all" value={newStudent.name} onChange={e => setNewStudent({...newStudent, name: e.target.value})} />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase text-[#FF4FA1] ml-1">Active Contact</label>
                  <input required type="tel" placeholder="+233..." className="w-full px-8 py-5 bg-gray-100 dark:bg-gray-800 rounded-2xl outline-none focus:ring-4 ring-[#FF4FA1]/10 font-bold transition-all" value={newStudent.phone} onChange={e => setNewStudent({...newStudent, phone: e.target.value})} />
                </div>
              </div>
              ...
              <div className="p-8 bg-gray-900 dark:bg-black rounded-[2rem] text-white space-y-6 shadow-2xl">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[9px] font-bold text-gray-500 uppercase">Preview: 100 Rooms</span>
                </div>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-16 pr-8 py-4 bg-white/10 rounded-xl outline-none font-bold text-sm focus:bg-white/20 transition-all border border-white/5"
                ...
                <div className="grid grid-cols-4 md:grid-cols-6 gap-3">
                  {availableRoomsList.map(r => (
                    <button 
                      ...
                      className={`py-3 rounded-xl font-black text-xs transition-all border-2 ${newStudent.room === r.roomNumber ? 'bg-[#00CFFF] border-[#00CFFF] text-black scale-110 shadow-lg' : 'bg-transparent border-white/10 hover:border-[#00CFFF]'}`}
                    >
                      {r.roomNumber}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["Male", "Female"].map(g => (
                        <button key={g} type="button" onClick={() => setNewStudent({...newStudent, gender: g})} className={`py-4 rounded-xl font-black text-xs uppercase tracking-widest border-2 transition-all ${newStudent.gender === g ? 'border-[#FF4FA1] bg-[#FF4FA1]/5 text-[#FF4FA1]' : 'border-gray-100 dark:border-gray-800'}`}>{g}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Registration Status</label>
                  <select className="w-full px-8 py-4 bg-gray-100 dark:bg-gray-800 rounded-xl font-bold border-none" value={newStudent.status} onChange={e => setNewStudent({...newStudent, status: e.target.value as any})}>
