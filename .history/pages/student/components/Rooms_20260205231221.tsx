...
const Rooms: React.FC = () => {
  ...
  const handleDelete = () => {
    if (selectedRoom) {
      ...
      addToast(`Room ${selectedRoom.roomNumber} removed`, "error");
      setActiveModal(null);
    }
  };
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter(room => 
      ...
      room.student.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="fixed top-5 right-5 z-[60] flex flex-col gap-2">
        {toasts.map(t => (
          <div key={t.id} className={`px-4 py-3 rounded-lg shadow-lg text-white font-medium transform transition-all duration-300 animate-in slide-in-from-right-full ${t.type === 'success' ? 'bg-green-600' : t.type === 'error' ? 'bg-red-600' : 'bg-blue-600'}`}>
            {t.text}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {["Room Number", "Type", "Status", "Price", "Occupant", "Actions"].map((label) => (
                  <th key={label} className="text-left py-3 px-4 text-sm font-medium text-gray-600 dark:text-gray-400">
                    {label}
                  </th>
                ))}
              </tr>
        ...
        <div className="flex items-center justify-between mt-6">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            Showing {currentData.length} rooms
          </div>
          <div className="flex items-center gap-2">
            <button 
      ...
      {activeModal === "view" && selectedRoom && (
        <Modal title={`Room ${selectedRoom.roomNumber} Details`} onClose={() => setActiveModal(null)}>
          <div className="space-y-4 text-gray-600 dark:text-gray-300">
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs uppercase font-bold text-gray-400">Type</p><p>{selectedRoom.type}</p></div>
              <div><p className="text-xs uppercase font-bold text-gray-400">Price</p><p>{selectedRoom.price}</p></div>
              <div><p className="text-xs uppercase font-bold text-gray-400">Status</p><p>{selectedRoom.status}</p></div>
              <div><p className="text-xs uppercase font-bold text-gray-400">Occupant</p><p>{selectedRoom.student}</p></div>
            </div>
            <button onClick={() => setActiveModal(null)} className="w-full mt-4 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg font-medium">Close</button>
          </div>
        </Modal>
      )}
      ...
      {activeModal === "delete" && selectedRoom && (
        <Modal title="Delete Room" onClose={() => setActiveModal(null)}>
          <p className="text-gray-600 dark:text-gray-400 mb-6">Are you sure you want to delete room <span className="font-bold text-gray-900 dark:text-white">{selectedRoom.roomNumber}</span>?</p>
          <div className="flex justify-end gap-3">
            <button onClick={() => setActiveModal(null)} className="px-4 py-2 text-gray-500">Cancel</button>
            <button onClick={handleDelete} className="px-4 py-2 bg-[#FF4FA1] text-white rounded-lg hover:bg-[#FF4FA1]/90">Delete</button>
          </div>
        </Modal>
      )}
