...
const Rooms: React.FC = () => {
  ...
  /* --- ADD ROOM FUNCTION COMMENTED OUT ---
  const openAdd = () => {
  ...
  */
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Add logic disabled as modal trigger is commented out
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between mt-6">
          <p className="text-sm text-gray-500">Showing {currentData.length} of {filteredRooms.length} rooms</p>
          <div className="flex items-center gap-2">
            <button 
        ...
        <Modal title="Edit Room Data" onClose={() => setActiveModal(null)}>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <button type="submit" className="px-6 py-2 bg-[#00CFFF] text-white rounded-lg font-bold shadow-lg shadow-[#00CFFF]/20">Save Changes</button>
            </div>
          </form>
        </Modal>
      )}
      ...
      {activeModal === "view" && selectedRoom && (
        <Modal title={`Room ${selectedRoom.roomNumber} Overview`} onClose={() => setActiveModal(null)}>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-4 border border-gray-100 dark:border-gray-700 rounded-xl">
               <div className="w-12 h-12 bg-[#00CFFF]/10 text-[#00CFFF] flex items-center justify-center rounded-full text-xl"><FiUser /></div>
               <div><p className="text-xs text-gray-400">Primary Occupant</p><p className="font-bold dark:text-white">{selectedRoom.student}</p></div>
            </div>
      ...
      {activeModal === "delete" && selectedRoom && (
        <Modal title="Confirm Removal" onClose={() => setActiveModal(null)}>
          <div className="text-center py-4">
            ...
            <p className="text-gray-600 dark:text-gray-400">You are about to delete <span className="font-bold text-gray-900 dark:text-white">Room {selectedRoom.roomNumber}</span>. This action cannot be undone.</p>
          </div>
          <div className="flex justify-center gap-3 mt-6">
            <button onClick={() => setActiveModal(null)} className="px-6 py-2 text-gray-500">Cancel</button>
            <button onClick={handleDelete} className="px-6 py-2 bg-[#FF4FA1] text-white rounded-lg font-bold shadow-lg shadow-[#FF4FA1]/20">Confirm Delete</button>
          </div>
        </Modal>
      )}
    </div>
